...
#####select idx#######
top_idx<-sort(p.est,decreasing = T)
select_idx<-names(top_idx[1:10])
sum(p.est[select_idx])
gplus <- (1 - theta) / theta
g.est <- p.est * gplus
...
for (j in 1:n_sim) {
  ...
  xb  = scale2(X[,1] + X[,2])*0.5 + scale2(rowSums(OTU))*b
  y <-  rbinom(n_sample, 1, prob=1/(1+exp(-xb)))
  ...
  p_MiRKAT[j]<-MiRKAT(y, X, Ks = Ks, out_type="D", method = "davies")$omnibus_p
  p_MiSPU[j]<-MiSPU(y, comm, throat.tree, cov = X, model ="binomial",
  ...
  p_anova_Du[j]<-tradition_preanova(n_rep=1000,d_mat=Du,predictor = y, confonding_stat = T,
                     confonding_var = X)
...
sum(as.numeric(p_anova_Du<0.01),na.rm = T)
